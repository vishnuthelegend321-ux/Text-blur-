...
# ==============================================================================
# Core Image Processing Function (MODIFIED)
# ==============================================================================
def blur_text_in_image(image_bytes):
    """
    Detects and blurs text in an image.
    If no text is found, it returns the original image.
    On processing error, it returns None.
    """
    try:
    ...
    if st.button(f"Start Processing {len(uploaded_files)} Images", type="primary"):
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                ...
                # This function now returns either the blurred image or the original image
                processed_image = blur_text_in_image(image_bytes)
                
                if processed_image is not None:
                    # Convert processed image (blurred OR original) back to bytes (PNG format)
                    pil_img = Image.fromarray(processed_image)
                    img_buffer = io.BytesIO()
                    pil_img.save(img_buffer, format="PNG")
                    
                    # Add the result to the zip file
                    zip_file.writestr(f"processed_{uploaded_file.name}.png", img_buffer.getvalue())
                    processed_count += 1
                else:
                    status_text.warning(f"Skipped file due to a processing error: {uploaded_file.name}")
